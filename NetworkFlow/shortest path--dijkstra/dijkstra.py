...
def dijkstra(G, start, end=None):
    ...
    Q = PQDict(D)           # priority queue for tracking min shortest path      Cin ????
    P = {}                  # mapping of nodes to their direct predecessors      存边
    U = set(G.keys())       # unexplored nodes

    while U:                                    # nodes yet to explore
        (v, d) = Q.popitem()                    # node w/ min dist d on frontier           找到Cln最小的l
...
if __name__ == '__main__':
    ...
    graph_1={'n1':{},'n2':{'n1':2},'n3':{'n1':9,'n2':8},'n4':{'n3':1,'n2':7}}
    # get shortest path distances to each node in `graph` from `a`
    dist, pred = dijkstra(graph_1, 'n4')
    print(dist)
    print(pred)
...
if __name__ == '__main__':
    ...
    graph = {'a': {'b': 1},
             ...
             'c': {'d': 1},
             'd': {}}
    graph_1={'n1':{'n3':9},'n2':{'n3':8,'n4':7},'n3':{'n4':1},'n4':{}}
    # get shortest path distances to each node in `graph` from `a`
