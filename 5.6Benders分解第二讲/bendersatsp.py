...
class WorkerLP:
    ...
    def __init__(self, numNodes):
        ...
        cpx = cplex.Cplex()
        cpx.set_results_stream(None) #write to fiename
...
def bendersATSP(sepFracSols, filename):
    try:
        ...
        if sepFracSols == "1":
            ...
            raise ValueError('sepFracSols must be one of "0" or "1"')
        ...
        # This instruction is redundant: If MIP control callbacks are
        # registered, then by default CPLEX uses 1 (one) thread only.
        # Note that the current example may not work properly if more than
        # 1 threads are used, because the callback functions modify shared
        # global data.  We refer the user to the documentation to see how to
        # deal with multi-thread runs in presence of MIP control callbacks.
...
def usage():
    ...
    print("           File ../../../examples/data/atsp.dat used if")
